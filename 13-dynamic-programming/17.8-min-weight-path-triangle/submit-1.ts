...
export function minimumPathTotal(
  triangle: number[][],
  curRow = 0,
  curCol = 0,
  curSum = 0,
  ...
  const cacheKey = String([curCol, curRow, curSum]);

  if (!(cacheKey in cache)) {
    if (curRow === triangle.length) return curSum;

    // visit cur node
    let newSum = curSum + triangle[curRow][curCol];

    cache[cacheKey] = Math.min(
      minimumPathTotal(triangle, curRow + 1, curCol, newSum),
      minimumPathTotal(triangle, curRow + 1, curCol + 1, newSum)
    );
  }
