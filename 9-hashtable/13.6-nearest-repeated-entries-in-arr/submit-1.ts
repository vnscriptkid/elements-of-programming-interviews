...
export function findNearestRepetition(paragraph: string[]): number {
  let minDistance = Infinity;

  const wordToLatestIdx: Record<string, number> = {};

  for (let i = 0; i < paragraph.length; i++) {
    let word = paragraph[i];

    if (word in wordToLatestIdx) {
      minDistance = Math.min(minDistance, i - wordToLatestIdx[word]);
    }
    wordToLatestIdx[word] = i;
  }
