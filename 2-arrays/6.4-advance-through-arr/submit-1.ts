...
export function canReachEnd(maxAdvanceSteps: number[]) {
  let furthestIdxSoFar = 0,
    lastIdx = maxAdvanceSteps.length - 1;

  for (let i = 0; i <= furthestIdxSoFar && furthestIdxSoFar < lastIdx; i++) {
    furthestIdxSoFar = Math.max(furthestIdxSoFar, i + maxAdvanceSteps[i]);
  }

  return furthestIdxSoFar >= lastIdx;
}
