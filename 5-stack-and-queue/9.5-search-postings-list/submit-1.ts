...
export function setJumpOrderRecursively(
  node: PostingListNode | null,
  order = { value: 0 }
) {
  if (node !== null && node.order === -1) {
    node.order = order.value;
    order.value++;
    setJumpOrderRecursively(node.jump, order);
    setJumpOrderRecursively(node.next, order);
  }
}

export function setJumpOrderIteratively(node: PostingListNode) {
  const stack: (PostingListNode | null)[] = [node];

  let order = 0;
  while (stack.length) {
    const nodeOut = stack.pop()!;

    if (nodeOut && nodeOut.order === -1) {
      nodeOut.order = order;
      order++;
      stack.push(nodeOut.next);
      stack.push(nodeOut.jump);
    }
  }
}
